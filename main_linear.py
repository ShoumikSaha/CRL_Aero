...
def set_loader(opt):
    ...
    val_transform_fn = get_my_transforms()
    val_transform = MyTransform(val_transform_fn, split='val')
    print(f"Val Transforms: {val_transform}")

    train_dataset = globals()[opt.dataset](data_folder=opt.data_folder, transform=train_transform, split='train', label_name=opt.label_name)
    val_dataset = globals()[opt.dataset](data_folder=opt.data_folder, transform=val_transform, split='val', label_name=opt.label_name)
    test_dataset = globals()[opt.dataset](data_folder=opt.data_folder, transform=val_transform, split='test', label_name=opt.label_name)
    ...
    train_loader = torch.utils.data.DataLoader(
        train_dataset, batch_size=opt.batch_size, shuffle=True, num_workers=opt.num_workers, pin_memory=True, drop_last=True
    )
    val_loader = torch.utils.data.DataLoader(
        val_dataset, batch_size=opt.batch_size, shuffle=False, num_workers=opt.num_workers, pin_memory=True, drop_last=True
    )
    test_loader = torch.utils.data.DataLoader(
        test_dataset, batch_size=opt.batch_size, shuffle=False, num_workers=opt.num_workers, pin_memory=True, drop_last=True
    )
...
def train(train_loader, model, regressor, criterion, optimizer, epoch, opt):
    ...
    for idx, (images, labels) in enumerate(train_loader):
        ...
        out1 = regressor(f1)
        out2 = regressor(f2)
        output = torch.cat([out1.unsqueeze(1), out2.unsqueeze(1)], dim=1)
